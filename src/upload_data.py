...
def find_nested_data(redis_client, redis_key, data):
    """
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            find_nested_data(redis_client, f"{redis_key}:{key}", value)
    elif isinstance(data, list):
        # If it's a list, iterate through the elements
        for index, element in enumerate(data):
            # Recursively store nested data
            find_nested_data(redis_client, f"{redis_key}[{index}]", element)
    else:
        # If it's a leaf node, store the value
        # redis_client.hset(redis_key, "value", json.dumps(data))
        print(f"value {str}", json.dumps(data))
# %%
"""
...
for i, xml_file_path in enumerate(xml_file_paths):
    ...
    print(find_nested_data(None, "key", dict_file))
    break
